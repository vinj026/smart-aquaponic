  ...
  >
    ...
    <div class="flex items-center justify-between mb-1.5">
      <div class="flex items-center gap-1.5">
        <div v-if="$slots.icon" class="text-gray-500 dark:text-gray-400 w-3.5 h-3.5 flex items-center justify-center">
          <slot name="icon" />
        </div>
        <span class="text-[10px] font-bold text-gray-600 dark:text-gray-400 uppercase tracking-widest">{{ label }}</span>
      </div>
    ...
    <div class="flex items-baseline gap-1 mt-1">
      ...
      <span v-if="unit" class="text-[10px] font-medium text-gray-500 dark:text-gray-400">{{ unit }}</span>
    </div>
  </div>
</template>
