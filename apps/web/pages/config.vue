    ...
    <div class="mx-auto relative z-10 px-4 py-8 space-y-6 transition-all duration-500" :class="isDesktopLayout ? 'max-w-4xl' : 'max-w-md'">
      ...
      <header class="flex items-center justify-between bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 px-3 py-2 rounded-md shadow-sm transition-colors duration-300">
        <div class="flex items-center gap-3">
          ...
          <h1 class="text-lg md:text-xl font-bold uppercase tracking-wider text-gray-900 dark:text-gray-100 border-l border-gray-100 dark:border-slate-700 pl-3">System Config</h1>
        </div>
        <div class="flex items-center gap-3">
      ...
      <div class="space-y-6">
        ...
        <section class="space-y-4 border-b border-gray-200 dark:border-slate-800 pb-6">
          <div v-if="configError" class="rounded-md border border-red-200 dark:border-red-900/50 bg-red-50/50 dark:bg-red-950/10 px-3 py-2 text-xs md:text-sm text-red-700 dark:text-red-300">
            Failed to load lifecycle configuration.
          </div>

          <div class="space-y-1">
            <h2 class="text-sm md:text-base font-medium text-gray-900 dark:text-gray-100">Lifecycle Settings</h2>
            <p class="text-xs md:text-sm text-gray-500 dark:text-gray-400 mt-1 mb-4">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="space-y-2">
              ...
              <input 
                ...
                type="date" 
                class="w-full bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-md px-3 py-2 text-xs md:text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 transition-all"
              />
            </div>
          ...
          <div class="flex justify-end">
            <button 
              ...
              class="inline-flex w-auto items-center justify-center rounded-md border border-gray-300 dark:border-slate-700 bg-transparent px-6 py-2 text-xs md:text-sm font-semibold text-gray-900 dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-slate-800 active:scale-[0.98] transition-all disabled:cursor-not-allowed disabled:opacity-50 disabled:scale-100"
            >
        ...
        <section class="space-y-4">
          <div class="space-y-1 px-1">
            <h2 class="text-sm md:text-base font-medium text-gray-900 dark:text-gray-100">Sensor Thresholds</h2>
            <p class="text-xs md:text-sm text-gray-500 dark:text-gray-400">
              These thresholds determine when sensor readings are flagged as warning or danger.
            </p>
          </div>

          <div v-if="thresholdsError" class="rounded-md border border-red-200 dark:border-red-900/50 bg-red-50/50 dark:bg-red-950/10 px-3 py-2 text-xs md:text-sm text-red-700 dark:text-red-300">
            Failed to load sensor thresholds.
          </div>

          <div v-else-if="thresholdsLoading && thresholds.length === 0" class="rounded-md border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 py-4 text-xs md:text-sm text-gray-500 dark:text-gray-400 text-center">
