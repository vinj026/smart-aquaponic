    ...
    <div class="mx-auto relative z-10 px-4 py-8 space-y-6 transition-all duration-500" :class="isDesktopLayout ? 'max-w-4xl' : 'max-w-md'">
      ...
      <header class="flex items-center justify-between bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 px-3 py-2 rounded-md shadow-sm transition-colors duration-300">
        <div class="flex items-center gap-3">
          ...
          <h1 class="text-lg font-bold uppercase tracking-wider text-gray-900 dark:text-gray-100 border-l border-gray-100 dark:border-slate-700 pl-3">Event Logs</h1>
        </div>
        <div class="flex items-center gap-3">
      ...
      <section class="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-md p-6 space-y-6 shadow-sm transition-colors duration-300">
        <div class="space-y-1">
          <h2 class="text-sm font-medium text-gray-900 dark:text-gray-100">System Logs</h2>
        ...
        <div v-else class="space-y-4">
          <div v-for="evt in events" :key="evt.id" class="flex flex-col gap-1 border-l-2 pl-3 pb-1 relative" :class="borderColorClass(evt.type)">
            ...
            <div class="flex flex-wrap items-baseline justify-between gap-x-2">
              ...
              <span class="text-[11px] text-gray-400 dark:text-gray-500 font-mono" :title="new Date(evt.timestamp).toLocaleString()">{{ timeAgo(evt.timestamp) }}</span>
            </div>
